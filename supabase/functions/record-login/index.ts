// Record login with IP geolocation - Self-contained version
...
function parseUserAgent(userAgent: string): { browser: string; os: string; deviceType: 'desktop' | 'mobile' | 'tablet' } {
...
async function getGeoLocation(ip: string): Promise<GeoIPResponse | null> {
  ...
  try {
    const response = await fetch(`${GEOIP_URL}/${ip}?fields=status,country,regionName,city,lat,lon`);
...
function getClientIP(req: Request): string | null {
  const headers = ['cf-connecting-ip', 'x-real-ip', 'x-forwarded-for', 'x-client-ip', 'true-client-ip'];
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('record-login invoked');
